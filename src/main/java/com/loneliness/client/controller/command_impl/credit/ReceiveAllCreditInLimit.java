...
public class ReceiveAllCreditInLimit implements Command<int[], Map<Integer, Credit>> {
    @Override
    public Map<Integer, Credit> execute(int[] request) throws ControllerException {
        try {
            return FactoryService.getInstance().getCreditService().receiveAllElemInLimit(request[0],request[1]);
