...
public class UpdateReportingPeriod implements Command<ReportingPeriod,String> {
    @Override
    public String execute(ReportingPeriod request) {
        return ServiceFactory.getInstance().getReportingPeriodService().update(request);
    }
}
